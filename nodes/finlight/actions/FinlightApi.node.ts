  ...
  async execute(this: IExecuteFunctions) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (operation === "search") {
        ...
        const sourcesRaw =
          (additional.sources?.sourceList as Array<{ source: string }>) || [];
        ...
        const optInSourcesRaw =
          (additional.optInSources?.sourceList as Array<{ source: string }>) ||
          [];
        if (optInSourcesRaw.length)
          body.optInSources = optInSourcesRaw.map((s) => s.source);

        const tickersRaw =
          (additional.tickers?.tickerList as Array<{ ticker: string }>) || [];
        if (tickersRaw.length) body.tickers = tickersRaw.map((t) => t.ticker);

        const countriesRaw =
          (additional.countries?.countryList as Array<{ country: string }>) ||
          [];
        if (countriesRaw.length)
          body.countries = countriesRaw.map((c) => c.country);

        if (
          Array.isArray(additional.categories) &&
          additional.categories.length
        ) {
          body.categories = additional.categories;
        }

        if (additional.language !== undefined)
          body.language = additional.language as string;
        if (additional.order !== undefined)
          body.order = additional.order as string;
        if (additional.orderBy !== undefined)
          body.orderBy = additional.orderBy as string;
        ...
        const includeContent = this.getNodeParameter(
          "getByLinkIncludeContent",
          i,
        ) as boolean;
        const includeEntities = this.getNodeParameter(
          "getByLinkIncludeEntities",
          i,
        ) as boolean;
        ...
        if (Array.isArray(response)) {
          returnData.push(
            ...response.map((s) => (typeof s === "string" ? { source: s } : s)),
          );
        } else {
